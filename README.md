...
Порт сервиса, параметры подключения к PostgreSQL и окружение задаются в  через локальные переменные (`environment:` в `docker-compose.yml` или `.env` файл). 
...
* **`make build`**
  Собирает бинарник приложения в директорию `bin/`.
...
* **`make test`**
  Запускает все тесты: юнит, E2E и интеграционные.

* **`make docker`**
  Собирает Docker-образ для приложения.

* **`make up`**
  Запускает контейнеры с `docker-compose` в фоновом режиме.

* **`make up-build`**
  Собирает и запускает контейнеры с пересборкой образов.

* **`make down`**
  Останавливает и удаляет контейнеры.

* **`make down-v`**
  Останавливает и удаляет контейнеры и volumes (сбрасывает БД).

* **`make logs`**
  Выводит логи приложения из Docker контейнера.

# Тесты
Проведены 
- интеграционные тесты для реализации репозиториев 
- юнит-тесты для внутренней бизнес-логики. 
- E2E тесты для всего приложения
- нагрузочное тестирование (результаты см. в конце)


Запуск всех тестов производится командой `go test ./...` из корня проекта.
...
/config               - Структура для получения переменных окружения.
/test                 - Папка с интеграционными, E2E и нагрузочным тестированием.
```
